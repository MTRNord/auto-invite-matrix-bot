...
async fn do_stuff(
    ...
    let session = Session {
        ...
        access_token: server.access_token.as_ref().expect("missing access_token").to_owned(),
        user_id: UserId::try_from(server.mxid.as_str()).unwrap(),
    };

    let client = HttpsClient::https(Url::parse(server.address.as_str()).expect("unable to parse url"), Some(session));
